...
import { collectProjectDiagnostics, printDiagnostics, summarizeDiagnostics, } from "../checker/index.js";
import { diagnosticSignatures, computeSuspects, healFiles, normPath, } from "./heal.js";
export class CheckRunner {
    ...
    async runOnce() {
        const tsFiles = [...this.deps.getTsFiles()].filter((f) => !f.endsWith('.d.ts'));
        if (tsFiles.length === 0) {
            this.deps.debugLog('Diagnostic check skipped — no .ts files');
            return;
        }
        const checkStart = Date.now();
        this.deps.debugLog(`Diagnostic check starting (${tsFiles.length} file(s), program=${this.deps.getProgram() ? 'reused' : 'fresh'})`);
        const firstResult = await this.collect(tsFiles);
        const { result, rewrote } = await this.healAndRecheck(firstResult, tsFiles);
        this.deps.debugLog(`Diagnostic check finished in ${Date.now() - checkStart}ms`);
     ...
     * still converging.
     */
    async healAndRecheck(result, tsFiles) {
        ...
        const carried = [...this.pendingHealSuspects].filter((f) => projectTsFiles.has(f) && !batch.has(f));
        ...
        if (suspects.length > 0) {
            this.deps.debugLog(`Heal: ${suspects.length} suspect file(s) with changed diagnostics`);
            ...
            if (rewrote.length > 0) {
                ...
                this.deps.debugLog(`Heal rewrote ${rewrote.length} file(s) — re-running diagnostic check once`);
                ...
                if (next.length > 0) {
                    for (const f of next)
                        this.pendingHealSuspects.add(f);
                    this.deps.debugLog(`Heal: ${next.length} file(s) affected by heal-writes — recheck scheduled`);
                    this.deps.requestRecheck();
                }
            }
        }
