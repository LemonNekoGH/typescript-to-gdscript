...
import { createLintOverlay } from "./lint.js";
/**
...
function init({ typescript: ts }) {
    function create(info) {
         ...
         */
        const debugLogPath = typeof pluginConfig.debugLog === 'string' &&
        ...
        try {
            ...
            log(`resolved config for ${projectDir}: tsDir=${cfg.tsDir} gdDir=${cfg.gdDir} cacheDir=${cfg.cacheDir}`);
        }
        catch (err) {
            log(`plugin disabled (resolveConfig / ProjectCache failed for ${projectDir}): ${err instanceof Error ? err.message : String(err)}`);
        }
        /**
        ...
        // on them.
        function filterAugmentationDiagnostics(fileName, diags) {
        ...
        proxy.getSyntacticDiagnostics = (fileName) => {
            ...
            return filterAugmentationDiagnostics(fileName, base);
        };
        ...
        if (cfg && cache) {
            ...
            proxy.getSemanticDiagnostics = (fileName) => {
                const base = filterAugmentationDiagnostics(fileName, ls.getSemanticDiagnostics(fileName));
            ...
            proxy.getSemanticDiagnostics = (fileName) => {
                return filterAugmentationDiagnostics(fileName, ls.getSemanticDiagnostics(fileName));
            };
        }
        return proxy;
    }
