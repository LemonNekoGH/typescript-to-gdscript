...
import { ProjectCache, hashContent, } from "../cache/index.js";
...
export function createLintOverlay(deps) {
     ...
     * on-disk state.
     */
    const lastGodotByFile = new LRU(100);
    /**
     ...
     */
    function cachedToTsDiagnostics(fileName, cached) {
        ...
        for (const d of cached) {
            if (resolve(d.file) !== resolve(fileName))
                continue;
            if (d.severity === 'info')
                continue;
            ...
            try {
                ...
                try {
                    start = ts.getPositionOfLineAndCharacter(sf, Math.min(line, sf.getLineStarts().length - 1), 0);
                }
                catch {
                    start = 0;
                }
            ...
            const nextLineStart = line + 1 < lineStarts.length ? lineStarts[line + 1] : sf.getEnd();
    ...
    function toCachedDiagnostics(diags) {
        return diags.map((d) => ({
            message: d.message,
            severity: d.severity,
            file: d.file,
    ...
    function cachedDiagnosticsFor(fileName) {
    ...
    function liveDiagnosticsFor(fileName) {
        ...
        sf.version ?? hashContent(sf.getFullText());
        ...
        if (memo && memo.version === version) {
            trace(`memo hit ${fileName} version=${version} converter=${memo.converter.length} godot=${memo.godot.length} → returning ${memo.converter.length + memo.godot.length} ts.Diagnostic(s)`);
            return [...memo.converter, ...memo.godot];
        }
        if (memo) {
            trace(`memo MISS ${fileName} had version=${memo.version} current=${version}`);
        }
        else {
        ...
        try {
            ...
            log(`convertTsToGd failed for ${fileName}: ${err instanceof Error ? err.message : String(err)}`);
            return [];
        }
        trace(`converter produced ${result.diagnostics.length} diag(s) for ${fileName}`);
        const converterDiags = cachedToTsDiagnostics(fileName, toCachedDiagnostics(result.diagnostics));
        trace(`mapped to ${converterDiags.length} ts.Diagnostic(s) (after current-file filter + severity=info drop)`);
        ...
        try {
            ...
            const relPath = relative(cfg.tsDir, fileName);
            const gdPath = resolve(cfg.gdDir, relPath.replace(/\.ts$/, '.gd'));
            cache.updateTsToGd(fileName, gdPath, result.sourceMap ?? '{}', result.diagnostics, { tsContent: bufferText, gdContent: result.code });
            cache
                .saveAsync()
                .catch((err) => {
                log(`cache saveAsync failed for ${fileName}: ${err instanceof Error ? err.message : String(err)}`);
            })
                .finally(() => {
            ...
            log(`cache persist failed for ${fileName}: ${err instanceof Error ? err.message : String(err)}`);
        }
        return [...entry.converter, ...entry.godot];
    }
    function maybeStartGodotValidation(fileName, version, convertResult) {
        ...
        try {
            ...
            trace(`godot: skip ${fileName} — resolveGodotPath threw: ${err instanceof Error ? err.message : String(err)}`);
            return;
        }
        ...
        validateGdFiles({
            ...
            if (pendingGodot.get(fileName) === ctrl)
                pendingGodot.delete(fileName);
            trace(`godot: result ${fileName} available=${validate.godotAvailable} diagnostics=${validate.diagnostics.length} ` +
            ...
            trace(`godot: stored ${current.godot.length} ts.Diagnostic(s) for ${fileName}; nudging tsserver`);
            ...
            try {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const refresh = info.project.refreshDiagnostics;
                ...
                trace(`godot: refreshDiagnostics threw: ${err instanceof Error ? err.message : String(err)}`);
            }
        }, (err) => {
            // Same settle cleanup as the success path.
            if (pendingGodot.get(fileName) === ctrl)
                pendingGodot.delete(fileName);
            log(`Godot validation failed for ${fileName}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
        });
    }
    return {
