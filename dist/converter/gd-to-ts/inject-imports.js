...
import { parseGdClassInfo } from "./index.js";
 ...
 */
export function injectMissingImports(files, userClasses, options) {
    if (options.generateGlobalClassTypes)
        return;
    if (files.length === 0)
        return;
    ...
    for (const file of files) {
        ...
        const importLines = sortedNames.map((name) => {
            ...
            const targetTsPath = gdToTs.get(normalizePath(info.filePath)) ??
 ...
 */
export function injectMissingImportsForProject(files, projectSources, registry, options) {
    if (options.generateGlobalClassTypes)
        return;
...
// ─── Helpers ────────────────────────────────────────────────
function collectMissingClassNames(diags, userClasses, file) {
    const out = new Set();
    for (const d of diags) {
        ...
        const info = userClasses.get(name);
        if (!info?.filePath)
            continue; // unknown class — not ours to fix
        if (sameFile(info.filePath, file.gdPath))
            continue; // self
        out.add(name);
    }
    return out;
 ...
 */
function importSpecifierFor(currentTs, targetTs) {
...
//# sourceMappingURL=inject-imports.js.map