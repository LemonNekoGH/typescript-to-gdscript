...
import { extractGdTypeName, inferExprType, isStaticFunction, } from "./type-inference.js";
/**
 ...
 *                   collected.
 */
export function buildClassScope(className, statements, ctx) {
    const scope = {
        className,
 ...
 */
function collectMember(child, scope, ctx) {
    ...
    if (child.type === SyntaxType.VariableStatement ||
        ...
        const inferredType = typeNode
            ...
            : valueNode
                ...
                : null;
        if (inferredType)
            scope.classMemberTypes.set(name, inferredType);
        return;
    }
 ...
 */
export function withClassScope(ctx, scope, fn) {
    const saved = {
