...
import { gdTypeToTs, containsAwait, extractGdTypeName, qualifyClassType, escapeSelfClassType, isStaticFunction, } from "./type-inference.js";
import { getAnnotations } from "./members.js";
import { emitBody } from "./statements.js";
import { emitExpr } from "./expressions.js";
import { isReferenceType } from "../common/index.js";
...
export function widenInType(rawType, tsType, ctx) {
    if (!tsType)
        return tsType;
    if (typeTextIncludesNull(tsType))
        return tsType;
    const firstSeg = rawType.split('.')[0] ?? rawType;
    // Skip enum references (numeric in TS).
    if (ctx.classEnumNames.has(firstSeg))
        return tsType;
    if (ctx.registry.isGlobalEnum(firstSeg))
        return tsType;
    ...
    if (!isReferenceType(rawType, ctx.registry))
        return tsType;
    return `${tsType} | null`;
}
// ─── Functions ────────────────────────────────────────────────
export function emitFunction(node, ctx, isAbstract = false) {
...
export function emitConstructor(node, ctx) {
    const paramsNode = node.childForFieldName('parameters');
    const bodyNode = node.childForFieldName('body');
...
export function collectParamNames(paramsNode, ctx) {
    if (!paramsNode)
        return;
    for (const child of paramsNode.namedChildren) {
        if (child.type === SyntaxType.Identifier) {
            ...
            const name = child.namedChildren.find((c) => c.type === SyntaxType.Identifier)?.text;
            if (name) {
                ...
                if (typeNode) {
                    ...
                    if (typeName)
                        ctx.localVarTypes.set(name, typeName);
                }
            }
        }
        else if (child.type === SyntaxType.VariadicParameter) {
            // Variadic param: `...args` or `...args: Array` — inner child holds the name
            const inner = child.namedChildren.find((c) => c.type === SyntaxType.Identifier ||
                c.type === SyntaxType.TypedParameter);
            const name = inner?.type === SyntaxType.Identifier
                ? inner.text
...
export function emitParams(paramsNode, ctx) {
    ...
    for (const child of paramsNode.namedChildren) {
        if (child.type === SyntaxType.Identifier) {
            ...
            const name = escapeTsBindingName(child.namedChildren.find((c) => c.type === SyntaxType.Identifier)
                ?.text ?? '');
            const typeNode = child.childForFieldName('type');
            const rawType = typeNode?.text ?? '';
            const baseType = escapeSelfClassType(qualifyClassType(rawType, ctx.classTypeNames, ctx.className) ??
                (typeNode ? gdTypeToTs(rawType) : null), ctx);
            ...
            if (inner?.type === SyntaxType.Identifier) {
                ...
                const name = escapeTsBindingName(inner.namedChildren.find((c) => c.type === SyntaxType.Identifier)
                    ?.text ?? '');
 ...
 */
export function emitReturnType(typeNode, ctx, isAsync = false) {
    let tsType;
    if (typeNode.type === SyntaxType.Type) {
        const inner = typeNode.namedChildren[0]?.text ?? typeNode.text;
        tsType = escapeSelfClassType(qualifyClassType(inner, ctx.classTypeNames, ctx.className) ??
            gdTypeToTs(inner), ctx);
    }
    else {
        const raw = typeNode.text;
        tsType = escapeSelfClassType(qualifyClassType(raw, ctx.classTypeNames, ctx.className) ??
            gdTypeToTs(raw), ctx);
    }
    if (!tsType)
        return '';
...
// ─── Lambda ───────────────────────────────────────────────────
export function emitLambda(node, ctx) {
    const paramsNode = node.childForFieldName('parameters');
    const returnTypeNode = node.childForFieldName('return_type');
    const bodyNode = node.childForFieldName('body');
    ...
    if (bodyNode && bodyNode.namedChildren.length === 1) {
        ...
        if (stmt.type === SyntaxType.ReturnStatement &&
            stmt.namedChildren.length > 0) {
            const expr = stmt.namedChildren[0];
            result = `${asyncPrefix}(${params})${returnType} => ${emitExpr(expr, ctx)}`;
        }
        else if (stmt.type === SyntaxType.ExpressionStatement &&
            stmt.namedChildren.length > 0) {
            const expr = stmt.namedChildren[0];
            result = `${asyncPrefix}(${params})${returnType} => { ${emitExpr(expr, ctx)}; }`;
        }
