...
import { escapeUnderscoreClassName } from "../common/index.js";
...
export function inferExprTypeStatic(node, registry) {
...
export function inferExprType(node, ctx) {
    ...
    if (node.type === SyntaxType.Integer)
        return 'int';
    if (node.type === SyntaxType.Float)
        return 'float';
    ...
    if (node.type === SyntaxType.BinaryOperator) {
        ...
        if (left)
            return inferExprType(left, ctx);
    }
    return null;
}
export const GD_OPS_MAP = {
    '+': 'add',
    '-': 'sub',
    '*': 'mul',
    '/': 'div',
 ...
 */
export function qualifyClassType(raw, classTypeNames, enclosingClassName) {
 ...
 */
export function selfClassNameIfMatches(raw, ctx) {
    if (!ctx.className || !raw)
        return null;
    if (raw === ctx.className)
 ...
 */
export function escapeSelfClassType(tsType, ctx) {
    if (!tsType)
        return tsType;
    return selfClassNameIfMatches(tsType, ctx) ?? tsType;
}
export function gdTypeToTs(gdType) {
    switch (gdType) {
        case 'int':
            return 'int';
        case 'float':
 ...
 */
export function splitTopLevelTypeArgs(inner) {
    const args = [];
    let depth = 0;
    let current = '';
