...
import { ANONYMOUS_ADDON_CLASS_NAME, gdFilenameToAnonymousClassName, escapeUnderscoreClassName, } from "../common/index.js";
import { resolveAllInheritedMembers, resolveInheritedMemberTypes, } from "./context.js";
import { buildClassScope, withClassScope } from "./class-scope.js";
import { emitFunction, emitConstructor } from "./functions.js";
import { emitClassVariable, emitSignal, emitEnum, emitComment, emitBlockComment, emitAnnotationAsDecorator, } from "./members.js";
import { emitFileScopeConst, emitFileScopeEnum, emitFileScopeClass, formatExtendsForTs, } from "./file-scope-emitter.js";
// ─── Source File ─────────────────────────────────────────────
export function emitSourceFile(root, ctx) {
    ...
    if (header.extendsClass) {
        const inherited = resolveAllInheritedMembers(header.extendsClass, ctx.userClasses, ctx.registry);
        for (const name of inherited)
            scope.classMembers.add(name);
        resolveInheritedMemberTypes(header.extendsClass, ctx.userClasses, scope.classMemberTypes);
    }
    ...
    const { memberLines, fileScopeLines } = withClassScope(ctx, scope, () => emitScriptClassBody(root, ctx, header));
    ...
    const classDecoratorLines = header.classLevelAnnotationNodes.map((node) => emitAnnotationAsDecorator(node, ctx));
    ...
    if (fileScopeLines.length > 0) {
        ...
        if (classDecoratorLines.length === 0)
            out.push('');
    }
 ...
 */
function resolveTsClassName(rawName, ctx) {
    if (!rawName) {
        return ctx.isAddon
 ...
 */
function scanScriptClassHeader(root) {
    let className = '';
    let extendsClass = '';
    let isAbstractClass = false;
    const rootAbstractAnnotationIndices = new Set();
    const abstractFunctionIndices = new Set();
    const abstractClassIndices = new Set();
    const classLevelAnnotationNodes = [];
    for (let i = 0; i < root.namedChildren.length; i++) {
        ...
        if (child.type === SyntaxType.ExtendsStatement) {
            ...
            if (child.text === '@abstract') {
                ...
                // above the header.
                if (next?.type === SyntaxType.ExtendsStatement ||
                    next?.type === SyntaxType.ClassNameStatement) {
                    ...
                    if (targetIndex !== -1)
                        abstractFunctionIndices.add(targetIndex);
                    rootAbstractAnnotationIndices.add(i);
                }
                else if (next?.type === SyntaxType.ClassDefinition) {
                    const targetIndex = indexOfNextNonAnnotationOrComment(root, i);
                    if (targetIndex !== -1)
                        abstractClassIndices.add(targetIndex);
                    rootAbstractAnnotationIndices.add(i);
                }
            }
 ...
 */
function nextNonAnnotationOrComment(root, start) {
 ...
 */
function indexOfNextNonAnnotationOrComment(root, start) {
    for (let j = start + 1; j < root.namedChildren.length; j++) {
        const candidate = root.namedChildren[j];
        if (candidate.type !== SyntaxType.Annotation &&
...
// emission.
function emitScriptClassBody(root, ctx, header) {
    const memberLines = [];
    const fileScopeLines = [];
    ...
    for (let ci = 0; ci < root.namedChildren.length; ci++) {
        ...
        if (child.type === SyntaxType.ExpressionStatement) {
            ...
            if (expr?.type === SyntaxType.String &&
        ...
        const isFunction = child.type === SyntaxType.FunctionDefinition ||
        ...
        if (child.type === SyntaxType.VariableStatement ||
            ...
            const hasSetget = child.namedChildren.some((c) => c.type === SyntaxType.Setget);
