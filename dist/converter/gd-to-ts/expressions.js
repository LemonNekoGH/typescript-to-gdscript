...
import { isGlobalName } from "./context.js";
import { inferExprType, selfClassNameIfMatches, qualifyClassType, GD_OPS_MAP, NOT_LIFT_OPS, GD_IS_PRIMITIVE_TYPES, } from "./type-inference.js";
import { emitLambda } from "./functions.js";
...
export function emitExpr(node, ctx) {
    if (node.type === SyntaxType.Identifier) {
        ...
        if (ctx.localVars.has(node.text))
            return escapeTsBindingName(node.text);
        ...
        if (!ctx.classMembers.has(node.text) &&
            ctx.registry.isGlobalFunction(node.text) &&
            sanitizeFunctionName(node.text) !== node.text) {
            ctx.diagnostics.push({
                message: `\`${node.text}\` is used as a value here, and TypeScript has no ` +
    ...
    if (node.type === SyntaxType.True)
        return 'true';
    if (node.type === SyntaxType.False)
        return 'false';
    if (node.type === SyntaxType.Null)
        return 'null';
...
// ─── Call Expression ──────────────────────────────────────────
export function emitCall(node, ctx) {
    ...
    if (callee.type === SyntaxType.Identifier &&
        callee.text === 'new' &&
        ctx.className &&
...
export function emitAttribute(node, ctx) {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (i === 0 &&
            ...
            const nextName = nextChild && nextChild.type === SyntaxType.Identifier
                ? nextChild.text
                : nextChild && nextChild.type === SyntaxType.AttributeCall
                    ? (nextChild.namedChildren.find((c) => c.type === SyntaxType.Identifier)?.text ?? '')
                    : '';
        ...
        if (child.type === SyntaxType.AttributeCall) {
            ...
            const methodName = child.namedChildren.find((c) => c.type === SyntaxType.Identifier)
                ?.text ?? '';
            ...
            if (methodName === 'new') {
                ...
                const className = qualifyClassType(rawClassName, ctx.classTypeNames, ctx.className) ??
            ...
            const attrName = child.namedChildren.find((c) => c.type === SyntaxType.Identifier)
                ?.text ?? '';
            const argsNode = child.childForFieldName('arguments');
            const key = argsNode?.namedChildren[0];
 ...
 */
export function tryEmitUniqueNodePath(node) {
...
export function emitBinaryOp(node, ctx) {
    const left = node.childForFieldName('left');
    const right = node.childForFieldName('right');
    const opNode = node.childForFieldName('op');
 ...
 */
function isGdLogicalValueContext(node) {
    const parent = node.parent;
    if (!parent)
        return false;
    // Assignment RHS: `a = b or c`
    if (parent.type === SyntaxType.Assignment ||
    ...
    if (parent.type === SyntaxType.Arguments)
        return true;
    // Return statement: `return a or b`
    if (parent.type === SyntaxType.ReturnStatement)
        return true;
    return false;
}
 ...
 */
function isGdBoolExpression(node) {
    if (node.type === SyntaxType.BinaryOperator) {
        const op = node.children.find((c) => !c.isNamed)?.text ?? '';
        if (GD_COMPARISON_OPS.has(op))
            return true;
        if (op === 'or' || op === 'and') {
            const left = node.childForFieldName('left');
            const right = node.childForFieldName('right');
            return ((left ? isGdBoolExpression(left) : true) &&
...
export function emitUnaryOp(node, ctx) {
