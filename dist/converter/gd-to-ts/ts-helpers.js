...
import { collectOperatorFixes } from "./helpers/operator-fix.js";
import { collectExplicitConvertFixes } from "./helpers/explicit-convert.js";
import { collectReadyFieldTypeFixes } from "./helpers/ready-field-types.js";
import { collectExtendsTypeFixes, collectUnsafeAnyFieldFixes, collectUnsafeNonNullFixes, } from "./helpers/unsafe-helpers.js";
import { collectNullableFixes } from "./helpers/nullable.js";
/**
...
export function runTsHelpers(options) {
    ...
    const result = { fixedFiles: [], diagnostics: [] };
    if (files.length === 0)
        return result;
    ...
    const runFixLoop = (collectors, label) => {
        for (let pass = 0; pass < MAX_FIX_PASSES; pass++) {
            ...
            for (const [fileName, fixes] of mergedFixes) {
                ...
                for (const f of fixes) {
                    const overlap = used.some((u) => (f.start >= u.start && f.start < u.end) ||
            ...
            if (label) {
                const breakdown = [...perHelper.entries()]
                    ...
                    .map(([name, n]) => `${name}: ${n}`)
                    .join(', ');
                console.log(`  [ts-helpers] ${label}: pass ${pass + 1} — ${fixedInPass} fix(es) applied (${breakdown})`);
            }
        }
    };
            ...
            ? [
                {
                    ...
                    collect: (program, filePaths) => collectExplicitConvertFixes(program, filePaths, registry),
                },
            ]
            : []),
        ...(registry
            ? [
                {
                    name: 'nullable',
                    collect: (program, filePaths) => collectNullableFixes(program, filePaths, registry, { addonMode }),
                },
            ]
            : []),
        {
            name: 'ready-field-types',
            collect: (program, filePaths) => collectReadyFieldTypeFixes(program, filePaths, registry, unsafeUseAny),
        },
        {
            name: 'extends-type',
            collect: (program, filePaths) => collectExtendsTypeFixes(program, filePaths),
        },
    ];
        ...
        runFixLoop([
            {
                ...
                collect: (program, filePaths) => collectUnsafeAnyFieldFixes(program, filePaths),
            },
        ], 'unsafe-any');
        ...
        runFixLoop([
            {
                ...
                collect: (program, filePaths) => collectUnsafeNonNullFixes(program, filePaths),
            },
        ], 'non-null');
    }
