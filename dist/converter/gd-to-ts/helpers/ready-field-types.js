 ...
 */
export function findReadyAssignment(cls, propName) {
    // Find _ready method
    const readyMethod = cls.members.find((m) => ts.isMethodDeclaration(m) &&
        !!m.name &&
 ...
 */
export function inferTypeFromExpression(expr, checker, sourceFile) {
    ...
    return checker.typeToString(type, expr, ts.TypeFormatFlags.NoTruncation | ts.TypeFormatFlags.UseFullyQualifiedType);
}
/**
...
export function isGdPrimitiveType(typeText, registry) {
    const t = typeText.trim();
    if (GD_BUILTIN_PRIMITIVE_TYPES.has(t))
        return true;
    // Strip generic params for checks like `Array<int>` -> `Array`
    const bare = t.replace(/<.*>$/, '').trim();
    if (GD_BUILTIN_PRIMITIVE_TYPES.has(bare))
        return true;
 ...
 */
function isEnumLikeType(typeNode, checker) {
 ...
 */
export function collectReadyFieldTypeFixes(program, filePaths, registry, unsafeUseAny) {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        function overlapsExisting(start, end) {
            return fixedRanges.some((r) => (start >= r.start && start < r.end) ||
                (end > r.start && end <= r.end) ||
                (start <= r.start && end >= r.end));
        }
        for (const diag of diagnostics) {
            if (!TS_CLASS_READY_ERROR_CODES.has(diag.code))
                continue;
            ...
            if (overlapsExisting(nameEnd, nameEnd + 1))
                continue;
            // Already has a `!` (definite-assignment assertion) or a `?` (optional)
            // -- skip in either case.
            if (propDecl.exclamationToken || propDecl.questionToken)
                continue;
...
//# sourceMappingURL=ready-field-types.js.map