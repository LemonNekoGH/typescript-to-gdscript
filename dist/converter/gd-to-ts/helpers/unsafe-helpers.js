...
import { findEnclosingClass } from "./ready-field-types.js";
// ─── Extends Type Helper ─────────────────────────────────────
/**
 ...
 */
export function findEnclosingMethod(node) {
    let current = node;
    while (current) {
 ...
 */
export function findParentMethodSignature(cls, methodName, checker) {
 ...
 * parameter types.
 */
export function collectExtendsTypeFixes(program, filePaths) {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        for (const diag of diagnostics) {
            if (diag.code !== TS_IMPLICIT_ANY_PARAM_CODE)
                continue;
            ...
            if (!method || !ts.isIdentifier(method.name))
                continue;
            ...
            if (!parentParamDecl || !ts.isParameter(parentParamDecl))
                continue;
            ...
            if (parentParamDecl.type) {
                ...
                const parentType = checker.getTypeOfSymbolAtLocation(parentParam, parentParamDecl);
                typeText = checker.typeToString(parentType, method, ts.TypeFormatFlags.NoTruncation |
 ...
 */
export function collectUnsafeAnyFieldFixes(program, filePaths) {
    const fixesByFile = new Map();
    for (const sourceFile of program.getSourceFiles()) {
        const fileName = sourceFile.fileName;
        if (!filePaths.has(fileName))
        ...
        const fixes = [];
        const pushFix = (start, replacement) => {
            if (fixedPositions.has(start))
                return;
 ...
 */
export function isNullAssignmentError(messageText) {
 ...
 */
export function collectUnsafeNonNullFixes(program, filePaths) {
    const fixesByFile = new Map();
    for (const sourceFile of program.getSourceFiles()) {
        const fileName = sourceFile.fileName;
        if (!filePaths.has(fileName))
        ...
        const pushNonNull = (pos) => {
            if (fixedPositions.has(pos))
                return;
            if (sourceText[pos] === '!')
                return; // already has `!`
        ...
        for (const diag of diagnostics) {
            ...
            if ((diag.code === 2322 || diag.code === 2345) &&
...
//# sourceMappingURL=unsafe-helpers.js.map