...
export const TS_COMPOUND_OP_TO_GD_OPS = {
...
export function findBinaryExpressionAt(sourceFile, pos) {
    function visit(node) {
 ...
 */
export function collectOperatorFixes(program, filePaths) {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        for (const diag of diagnostics) {
            if (!TS_OPERATOR_ERROR_CODES.has(diag.code))
                continue;
            ...
            if (overlapsExisting(exprStart, exprEnd))
                continue;
...
//# sourceMappingURL=operator-fix.js.map