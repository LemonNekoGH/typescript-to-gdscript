...
export const TS_TO_GD_TYPE_NAMES = new Map([
    ['string', 'String'],
    ['boolean', 'bool'],
    ['number', 'float'],
]);
 ...
 */
export function extractAssignmentTypes(messageText) {
    const text = typeof messageText === 'string' ? messageText : messageText.messageText;
    const notAssignable = text.match(/type '([^']+)' is not assignable to (?:parameter of )?type '([^']+)'/);
    if (notAssignable) {
        return {
    ...
    const missingProps = text.match(/Type '([^']+)' is missing the following properties from type '([^']+)'/);
    if (missingProps) {
        return {
    ...
    const missingProp = text.match(/Property '[^']+' is missing in type '([^']+)' but required in type '([^']+)'/);
    if (missingProp) {
        return {
 ...
 *   "Color[]"          -> "Array"
 */
export function simplifyTypeName(type) {
    let t = type
        .replace(/\s*\|\s*null$/, '')
 ...
 */
export function findNodeAt(sourceFile, pos, length) {
 ...
 */
export function collectExplicitConvertFixes(program, filePaths, registry) {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        for (const diag of diagnostics) {
            if (!TS_ASSIGNMENT_ERROR_CODES.has(diag.code))
                continue;
            ...
            if (!registry.canVariantConvert(gdSource, gdTarget))
                continue;
...
//# sourceMappingURL=explicit-convert.js.map