...
import { findNodeAt } from "./explicit-convert.js";
 ...
 */
function isTsReferenceType(typeNode, checker, registry) {
    const type = checker.getTypeAtLocation(typeNode);
    const excludeFlags = ts.TypeFlags.NumberLike |
    ...
    if (registry.isConstructor(firstSeg))
        return false;
    return true;
}
/** True when the type text already includes `null` as a union member. */
function typeTextIncludesNull(text) {
 ...
 */
function isTextReferenceType(text, registry) {
    const cleaned = text.trim();
    if (!cleaned)
        return false;
    if (NON_REFERENCE_NAMES.has(cleaned))
        return false;
 ...
 */
function hasAssignmentTo(body, propName) {
    if (!body)
        return false;
    let found = false;
    function visit(node) {
 ...
 */
function isPropertyAssigned(cls, prop, propName) {
    if (prop.initializer)
        return true;
 ...
 */
function collectPhaseAFixes(sourceFile, checker, registry) {
    const fixes = [];
    function visitClass(cls) {
        for (const member of cls.members) {
            ...
            if (!member.type)
                continue;
            if (member.exclamationToken || member.questionToken)
                continue;
            if (typeTextIncludesNull(member.type.getText(sourceFile)))
                continue;
            if (!isTsReferenceType(member.type, checker, registry))
                continue;
            if (isPropertyAssigned(cls, member, member.name.text))
                continue;
 ...
 */
function collectPhaseBFixes(sourceFile, checker, registry) {
    const fixes = [];
    function widen(typeNode) {
        if (!typeNode)
            return;
        ...
        const type = checker.getTypeAtLocation(typeNode);
        const hardExclude = ts.TypeFlags.EnumLike |
            ts.TypeFlags.NumberLike |
            ts.TypeFlags.StringLike |
            ts.TypeFlags.BooleanLike |
            ts.TypeFlags.Void |
            ts.TypeFlags.Never;
        if (type.flags & hardExclude)
            return;
        if (!isTextReferenceType(text, registry))
            return;
        fixes.push({
 ...
 * `Node | null`).
 */
function typeIncludesNull(type) {
 ...
 */
function findSourceExpression(node) {
    let current = node;
    while (current) {
...
function diagnosticHasNullSource(node, checker) {
    const sourceExpr = findSourceExpression(node);
    if (!sourceExpr)
        return false;
    if (sourceExpr.kind === ts.SyntaxKind.NullKeyword)
        return true;
 ...
 */
function findWidenTarget(node, checker, sourceFile) {
    let current = node;
    while (current) {
        if (ts.isReturnStatement(current)) {
...
function collectPhaseCFixes(program, sourceFile, registry) {
    ...
    for (const diag of diagnostics) {
        ...
        if (!diagnosticHasNullSource(node, checker))
            continue;
        ...
        if (usedEnds.has(end))
            continue;
        if (typeTextIncludesNull(typeNode.getText(sourceFile)))
            continue;
        if (!isTsReferenceType(typeNode, checker, registry))
            continue;
        usedEnds.add(end);
 ...
 */
function removeNullFromUnion(text) {
 ...
 */
function collectPhaseDFixes(program, sourceFile) {
    const checker = program.getTypeChecker();
    const diagnostics = program.getSemanticDiagnostics(sourceFile);
    const fixes = [];
    ...
    for (const diag of diagnostics) {
        ...
        if (diag.code !== 2531 && diag.code !== 18047)
            continue;
        if (diag.start === undefined || diag.length === undefined)
            continue;
        const node = findNodeAt(sourceFile, diag.start, diag.length);
        ...
        if (decl.parent && ts.isSetAccessorDeclaration(decl.parent))
            continue;
        const typeStart = decl.type.getStart(sourceFile);
        if (usedParams.has(typeStart))
            continue;
        const typeText = decl.type.getText(sourceFile);
        if (!typeTextIncludesNull(typeText))
            continue;
...
export function collectNullableFixes(program, filePaths, registry, options) {
    const fixesByFile = new Map();
    const checker = program.getTypeChecker();
    for (const sourceFile of program.getSourceFiles()) {
        if (!filePaths.has(sourceFile.fileName))
            continue;
        ...
        if (perFile.length > 0)
            fixesByFile.set(sourceFile.fileName, perFile);
    }
    return fixesByFile;
}
