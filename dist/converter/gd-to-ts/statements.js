...
export function emitBody(node, ctx, depth) {
    ...
    for (const child of node.namedChildren) {
        ...
        if (child.type === SyntaxType.ReturnStatement) {
            ...
            lines.push(value ? `${indent}return ${emitExpr(value, ctx)};` : `${indent}return;`);
            continue;
        }
...
// ─── Control Flow ─────────────────────────────────────────────
function emitIfStatement(node, ctx, depth) {
    const indent = '  '.repeat(depth);
    ...
    if (body)
        result += emitBody(body, ctx, depth + 1);
    result += `\n${indent}}`;
    ...
    for (const alt of alternatives) {
        if (alt.type === SyntaxType.ElifClause) {
            ...
            if (elifBody)
                result += emitBody(elifBody, ctx, depth + 1);
            result += `\n${indent}}`;
        }
            ...
            result += ` else {\n`;
            if (elseBody)
                result += emitBody(elseBody, ctx, depth + 1);
            result += `\n${indent}}`;
        }
    }
    return result;
}
function emitForStatement(node, ctx, depth) {
    const indent = '  '.repeat(depth);
...
function emitWhileStatement(node, ctx, depth) {
    const indent = '  '.repeat(depth);
    const condition = node.childForFieldName('condition');
    const body = node.childForFieldName('body');
    const condStr = condition ? emitExpr(condition, ctx) : 'true';
...
export function emitAugmentedAssignment(node, ctx) {
    const left = node.childForFieldName('left');
    const right = node.childForFieldName('right');
