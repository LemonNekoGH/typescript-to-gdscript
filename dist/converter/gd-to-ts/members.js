...
import { gdTypeToTs, extractGdTypeName, inferExprType, qualifyClassType, escapeSelfClassType, } from "./type-inference.js";
import { emitExpr } from "./expressions.js";
import { emitBody } from "./statements.js";
import { widenInType } from "./functions.js";
import { escapeTsBindingName } from "./identifiers.js";
// ─── Comments ─────────────────────────────────────────────────
/** Convert a triple-quoted GDScript string to a JS block comment */
export function emitBlockComment(text, indent) {
...
export function emitComment(node) {
...
// ─── Comments (inline) ───────────────────────────────────────
export function emitCommentInline(node) {
    const text = node.text;
    if (text.startsWith('##')) {
        const content = text.slice(2).trim();
        return `/** ${content} */`;
...
// ─── Signals ──────────────────────────────────────────────────
export function emitSignal(node, ctx) {
...
export function emitSignalParamTypes(paramsNode, ctx) {
    const types = [];
...
// ─── Enums ────────────────────────────────────────────────────
export function emitEnum(node, ctx) {
    ...
    if (!bodyNode)
        return '';
    const enumerators = bodyNode.namedChildren.filter((c) => c.type === SyntaxType.Enumerator);
...
// ─── Variables ────────────────────────────────────────────────
export function emitClassVariable(node, ctx) {
    ...
    const setgetNode = node.namedChildren.find((c) => c.type === SyntaxType.Setget);
    let decorators = '';
    ...
    if (setgetNode) {
        return emitSetgetVariable(name, typeNode, typeAnnotation, valueNode, setgetNode, decorators, staticPrefix, ctx);
    }
 ...
 */
function tryEmitTypeofValue(valueNode, ctx) {
 ...
 */
function emitSetgetVariable(name, typeNode, typeAnnotation, valueNode, setgetNode, decorators, staticPrefix, ctx) {
    const getNode = setgetNode.namedChildren.find((c) => c.type === SyntaxType.GetBody || c.type === SyntaxType.Getter);
    const setNode = setgetNode.namedChildren.find((c) => c.type === SyntaxType.SetBody || c.type === SyntaxType.Setter);
    ...
    if (hasAnyInline && hasAnyRef) {
        ctx.diagnostics.push({
            message: `Variable '${name}': cannot mix inline get/set bodies with ` +
    ...
    if (!useGdGetset) {
        ...
        return emitTsAccessors(name, accessorType, getNode, setNode, decorators, staticPrefix, ctx);
    }
    // Complex case: emit `name = gd.getset<T>({...})` assignment
    return emitGdGetsetCall(name, tsType, typeAnnotation, valueNode, getNode, setNode, decorators, staticPrefix, ctx);
}
/**
 ...
 */
function emitTsAccessors(name, tsType, getNode, setNode, decorators, staticPrefix, ctx) {
    const indent = '  ';
    const bodyIndent = '    ';
    const lines = [];
    ...
    if (setNode?.type === SyntaxType.SetBody) {
        ...
        const params = setNode.namedChildren.find((c) => c.type === SyntaxType.Parameters);
        const paramName = params?.namedChildren.find((c) => c.type === SyntaxType.Identifier)
            ?.text ?? 'value';
 ...
 */
function emitGdGetsetCall(name, tsType, typeAnnotation, valueNode, getNode, setNode, decorators, staticPrefix, ctx) {
    const indent = '  ';
    const iProp = '    ';
    const iBody = '      ';
    const lines = [];
...
export function emitLocalVariable(node, ctx, indent) {
    const name = node.childForFieldName('name')?.text ?? '';
    const typeNode = node.childForFieldName('type');
    const valueNode = node.childForFieldName('value');
    ...
    const inferredType = typeNode
        ...
        : valueNode
            ...
            : null;
    if (inferredType)
        ctx.localVarTypes.set(name, inferredType);
    const typeAnnotation = typeNode ? emitTypeAnnotation(typeNode, ctx) : '';
    const init = valueNode ? ` = ${emitExpr(valueNode, ctx)}` : '';
...
// ─── Type Annotations ─────────────────────────────────────────
export function emitTypeAnnotation(typeNode, ctx) {
...
export function getAnnotations(node) {
    const annotations = [];
...
export function emitAnnotationAsDecorator(node, ctx) {
