...
import { isAmbient } from "../common/gd-names.js";
/**
 ...
 */
function isNamedCallTarget(callee) {
 ...
 */
function declarationsOf(t, callee) {
 ...
 */
function isNamedFunction(d) {
    return (ts.isMethodDeclaration(d) ||
        ts.isMethodSignature(d) ||
        ts.isFunctionDeclaration(d) ||
        (ts.isVariableDeclaration(d) && isAmbient(d)));
}
function resolvesToNamedFunction(t, callee) {
 ...
 */
export function isCallableValueCall(t, callee) {
 ...
 */
export function isCallableMemberCall(t, callee) {
    const decls = declarationsOf(t, callee);
    if (decls.length === 0)
        return isCallableValueCall(t, callee);
    return !decls.some(isNamedFunction);
}
//# sourceMappingURL=callable-call.js.map