 ...
 */
export function isPlainObjectType(type, checker, registry) {
    const constituents = type.isUnion()
        ? type.types.filter((u) => !(u.flags & ts.TypeFlags.Null) && !(u.flags & ts.TypeFlags.Undefined))
        : [type];
    if (constituents.length === 0)
        return false;
    return constituents.every((c) => isPlainObjectConstituent(c, checker, registry));
}
function isPlainObjectConstituent(type, checker, registry) {
    ...
    for (const name of names) {
        ...
        if (name === 'Dictionary' || name === 'TSOnly')
            return false;
...
//# sourceMappingURL=access-rewrite.js.map