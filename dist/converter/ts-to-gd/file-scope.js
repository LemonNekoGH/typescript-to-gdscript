...
import { emitClassMembers } from "./class-body.js";
...
export function emitFileScopeVariable(node, t, opts) {
    const leadingBlank = opts?.leadingBlank ?? true;
    const flags = node.declarationList.flags;
    const isConst = (flags & ts.NodeFlags.Const) !== 0;
    if (!isConst) {
        t.addDiagnostic(node, 'error', 'File-scope `let` / `var` is not supported — use `const`, or move it inside the class as an instance / static member.');
        return;
    }
    for (const decl of node.declarationList.declarations) {
        if (!decl.name || !ts.isIdentifier(decl.name)) {
            t.addDiagnostic(decl, 'error', 'File-scope `const` must use a simple identifier (no destructuring patterns).');
            continue;
        }
        if (!decl.initializer) {
            t.addDiagnostic(decl, 'error', 'File-scope `const` must have an initializer (GDScript `const` requires a value).');
            continue;
        }
        const pos = t.getLineAndCol(decl);
        const name = decl.name.text;
        const gdType = decl.type
            ? tsTypeNodeToGdType(decl.type, t.ctx.checker, t.ctx.sourceFile, t.currentClassName, t.ctx.registry)
            : null;
...
export function emitFileScopeEnum(node, t, opts) {
    const leadingBlank = opts?.leadingBlank ?? true;
    ...
    for (const member of node.members) {
        ...
        if (ts.isStringLiteral(init) || ts.isNoSubstitutionTemplateLiteral(init)) {
            t.addDiagnostic(init, 'error', `Enum member "${memberName}" has a string value. GDScript enums only support integer values — remove the initializer or use a numeric one.`);
    ...
    t.emitter.writeLine(`enum ${name} { ${members.join(', ')} }`, pos.line, pos.col);
}
...
export function emitFileScopeClass(node, t, importMap, opts) {
    const leadingBlank = opts?.leadingBlank ?? true;
    const pairedStatements = opts?.pairedStatements;
    const name = node.name?.getText(t.ctx.sourceFile);
    if (!name) {
        t.addDiagnostic(node, 'error', 'File-scope class must have a name (no anonymous classes at file scope).');
        return;
    }
    const pos = t.getLineAndCol(node);
    ...
    if (!hasExtends) {
        t.addDiagnostic(node.name ?? node, 'type-error', 'Class has no `extends` clause. GDScript defaults to `RefCounted` — ' +
    ...
    const hasNamespaceContent = pairedStatements !== undefined &&
        pairedStatements.some((s) => ts.isVariableStatement(s) ||
    ...
    const hasAnyClassMember = node.members.some((m) => ts.isPropertyDeclaration(m) ||
...
export function emitFileScopeNamespace(node, scriptClass, t, importMap) {
    ...
    if (namespaceName !== expectedName) {
        t.addDiagnostic(node.name, 'error', `Namespace "${namespaceName}" does not match the script class "${expectedName}". File-scope namespaces must pair with the script class (their members lift into its body).`);
        return;
    }
    if (!node.body || !ts.isModuleBlock(node.body)) {
        ...
        t.addDiagnostic(node, 'error', 'Namespace must use a `{ ... }` block body (no qualified-name forms).');
        return;
    }
    ...
    const firstMatch = sf.statements.find((s) => ts.isModuleDeclaration(s) &&
 ...
 */
function collectNamespacesByName(statements) {
 ...
 *        layout expects.
 */
function emitNamespaceBody(statements, t, importMap, insideClassBody) {
    ...
    const consumedClassNames = new Set();
    for (const stmt of statements) {
        ...
        if (ts.isVariableStatement(stmt) ||
            ...
            if (!hasExportModifier(stmt)) {
                t.addDiagnostic(stmt, 'type-error', 'Namespace members must be `export`ed to resolve via TypeScript declaration merging. ' +
        ...
        if (ts.isClassDeclaration(stmt)) {
            ...
            const paired = className && !consumedClassNames.has(className)
                ? pairedByClassName.get(className)
                : undefined;
            if (className)
                consumedClassNames.add(className);
        ...
        if (ts.isModuleDeclaration(stmt)) {
            ...
            // error).
            const nsName = ts.isIdentifier(stmt.name) ? stmt.name.text : '';
            const hasSibling = statements.some((s) => ts.isClassDeclaration(s) && s.name?.text === nsName);
            if (!hasSibling) {
                // Only report on the FIRST unpaired namespace of a given
                // name — duplicates would just echo the same message.
                const firstUnpaired = statements.find((s) => ts.isModuleDeclaration(s) &&
                    ts.isIdentifier(s.name) &&
                    s.name.text === nsName);
                if (firstUnpaired === stmt) {
                    t.addDiagnostic(stmt.name, 'error', `Nested namespace "${nsName}" has no matching sibling class. Inner namespaces must pair with a class of the same name in the same scope.`);
                }
            }
            continue;
        }
        t.addDiagnostic(stmt, 'error', `Namespace member not supported: ${ts.SyntaxKind[stmt.kind]}`);
    }
}
...
export function collectLiftedNames(sf, scriptClass) {
    const names = new Set();
    ...
    const collectDirectMembers = (statements) => {
        for (const stmt of statements) {
            if (ts.isVariableStatement(stmt)) {
