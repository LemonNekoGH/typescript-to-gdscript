...
import { isSignalProperty, isEnumProperty, visitSignalDeclaration, visitEnumDeclaration, visitPropertyDeclaration, visitAccessorPair, visitConstructor, visitMethodDeclaration, getDecorators, } from "./class-members.js";
...
export function emitClassHeader(node, t, ctx) {
    ...
    for (const member of node.members) {
        if ((ts.isPropertyDeclaration(member) || ts.isMethodDeclaration(member)) &&
            ...
            if (ctx.importMap.has(name)) {
                t.addDiagnostic(member.name, 'error', `Field/method name "${name}" conflicts with an imported class of the same name. Rename the field or the import alias.`);
            }
            else if (ctx.liftedNames.has(name)) {
                t.addDiagnostic(member.name, 'error', `Field/method name "${name}" conflicts with a file-scope declaration that lifts into this class. Rename the field or the file-scope declaration.`);
            }
        }
    }
    return className;
}
export function emitClassMembers(node, t, opts) {
    ...
    for (const member of node.members) {
        if (ts.isPropertyDeclaration(member)) {
            ...
            if (!name)
                continue;
            const existing = orderedMembers.find((e) => e.kind === 'accessor' && e.name === name);
            if (existing) {
