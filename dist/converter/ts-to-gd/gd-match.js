...
export function visitGdMatchStatement(t, node) {
...
function emitGdMatchCase(t, obj) {
    let matchExpr;
    let matchManyExpr;
    let doBody;
...
function emitGdMatchArrowCase(t, arrow) {
 ...
 */
export function emitMatchPatternExpr(t, node, bindings, wildcardUndefined = true) {
    ...
    if (ts.isObjectLiteralExpression(node)) {
        ...
        for (const prop of node.properties) {
            ...
            if (ts.isPropertyAssignment(prop)) {
                ...
                const val = emitMatchPatternExpr(t, prop.initializer, bindings, wildcardUndefined);
                if (val === '_') {
    ...
    if (ts.isPropertyAccessExpression(node)) {
        ...
        const isOwnClass = root.kind === ts.SyntaxKind.ThisKeyword ||
...
//# sourceMappingURL=gd-match.js.map