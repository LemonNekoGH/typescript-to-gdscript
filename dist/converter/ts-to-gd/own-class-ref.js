 ...
 */
export function getEnclosingClass(node) {
 ...
 */
export function isStaticContext(node) {
    for (let n = node.parent; n; n = n.parent) {
        if (ts.isClassDeclaration(n) || ts.isClassExpression(n))
            return false;
        if (ts.isMethodDeclaration(n) ||
            ...
            return (n.modifiers?.some((m) => m.kind === ts.SyntaxKind.StaticKeyword) ??
                false);
        }
    }
    return false;
 ...
 */
export function getOwnClassName(node, scriptClassName) {
    const name = getEnclosingClass(node)?.name?.text;
    if (!name)
        return null;
    if (name === scriptClassName && isAnonymousClassName(name))
        return null;
    return name;
}
/** True when `pattern` binds `target`, destructuring included. */
function bindingBinds(pattern, target) {
    if (ts.isIdentifier(pattern))
        return pattern.text === target;
    return pattern.elements.some((el) => !ts.isOmittedExpression(el) && bindingBinds(el.name, target));
}
function declarationsBind(list, target) {
 ...
 */
function scopeBinds(n, target) {
    if (ts.isFunctionLike(n)) {
        return n.parameters.some((p) => bindingBinds(p.name, target));
    }
    if (ts.isCatchClause(n)) {
        return (n.variableDeclaration !== undefined &&
    ...
    if (ts.isBlock(n) || ts.isCaseClause(n) || ts.isDefaultClause(n)) {
        return n.statements.some((s) => (ts.isVariableStatement(s) &&
 ...
 */
function isShadowedByLocal(node, target) {
    for (let n = node.parent; n; n = n.parent) {
        if (ts.isClassDeclaration(n) || ts.isClassExpression(n))
            return false;
        if (scopeBinds(n, target))
            return true;
    }
    return false;
}
 ...
 */
function bareRef(node, memberName, scriptClassName) {
    if (!isShadowedByLocal(node, memberName))
        return { kind: 'prefix', text: '' };
    return {
        kind: 'unsupported',
        reason: `\`${memberName}\` is a member of \`${scriptClassName}\` and also a ` +
 ...
 */
export function resolveOwnClassRef(objExpr, memberName, scriptClassName) {
    if (objExpr.kind === ts.SyntaxKind.ThisKeyword) {
        if (!isStaticContext(objExpr))
            return FALLTHROUGH;
        const name = getOwnClassName(objExpr, scriptClassName);
        if (name !== null)
            return { kind: 'prefix', text: `${name}.` };
    ...
    if (getEnclosingClass(objExpr)?.name?.text !== scriptClassName) {
        return {
            kind: 'unsupported',
            reason: `\`${scriptClassName}.${memberName}\` can't be reached from inside ` +
    ...
    if (!isStaticContext(objExpr))
        return { kind: 'prefix', text: 'self.' };
    return bareRef(objExpr, memberName, scriptClassName);
}
//# sourceMappingURL=own-class-ref.js.map