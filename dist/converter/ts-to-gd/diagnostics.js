 ...
 */
export function buildDiagnosticsTypeInfo(registry) {
    const constructors = new Set();
    const packedArrayTypes = new Set();
    const bannedInTypes = new Set();
    if (registry) {
        for (const name of registry.getData().constructors ?? []) {
            constructors.add(name);
            if (GD_IN_ALLOWED_CONTAINER_TYPES.has(name))
                continue;
 ...
 */
export function isGdClassType(gdType, diagInfo) {
    if (GD_PRIMITIVE_TYPES.has(gdType))
        return false;
    if (diagInfo.constructors.has(gdType))
        return false;
    return true;
}
/**
 ...
 */
export function isGdVariantType(gdType, diagInfo) {
    if (GD_PRIMITIVE_TYPES.has(gdType))
        return false;
 ...
 */
export function classifyInRhsType(type, checker, diagInfo) {
