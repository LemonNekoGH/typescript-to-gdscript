...
import { isGdEvalCall, processGdEval, emitGdEval, isGdMatchCall, visitGdMatchStatement, emitMatchPatternExpr, } from "./gd-helpers.js";
...
export function visitStatement(t, node) {
    ...
    if (ts.isVariableStatement(node)) {
        ...
        if (isGdEvalCall(node.expression)) {
            ...
            visitGdMatchStatement(t, node.expression, visitStatement);
        }
        else {
...
// ---- Switch -> Match ----
export function visitSwitchStatement(t, node) {
    const pos = t.getLineAndCol(node);
    t.emitter.writeLine(`match ${t.emitExpression(node.expression)}:`, pos.line, pos.col);
    t.emitter.indent();
