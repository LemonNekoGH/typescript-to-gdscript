...
import { isGdEvalCall, processGdEval, emitGdEval } from "./gd-helpers.js";
import { isGdMatchCall, visitGdMatchStatement } from "./gd-match.js";
import { isVoidExpression } from "./void-value.js";
import { SWITCH_BREAK_ERROR, visitSwitchStatement } from "./switch.js";
import { emitStatements, isLabeledJump, isSwitchBreak, } from "./statement-body.js";
/**
 ...
 */
const LABEL_ERROR = 'Labels are not supported — GDScript has none, and a `break` or ' +
...
export function visitBlock(t, block) {
    emitStatements(t, block.statements, t.getLineAndCol(block), block.getLastToken());
}
export function visitStatement(t, node) {
    ...
    if (ts.isVariableStatement(node)) {
        ...
        if (isGdEvalCall(node.expression)) {
            emitGdEval(t, node.expression, pos);
        }
        else if (isGdMatchCall(node.expression)) {
            visitGdMatchStatement(t, node.expression);
        }
        else {
        ...
        t.addDiagnostic(node, 'error', '`for...in` is not supported; use `for...of` instead');
    }
    else {
        t.addDiagnostic(node, 'error', `Unsupported statement: ${ts.SyntaxKind[node.kind]}`);
    }
}
// ---- Variable Statements ----
export function visitVariableStatement(t, node) {
    for (const decl of node.declarationList.declarations) {
        const pos = t.getLineAndCol(decl);
        // Check for destructuring
        if (ts.isArrayBindingPattern(decl.name) ||
            ts.isObjectBindingPattern(decl.name)) {
            t.addDiagnostic(decl, 'error', 'Destructuring is not supported in GDScript');
            continue;
        }
        ...
        if (!(flags & (ts.NodeFlags.Const | ts.NodeFlags.Let))) {
            t.addDiagnostic(node, 'warning', '`var` is restricted; use `let` or `const` instead. Converting to `var`.');
        }
        // Type
        ...
        if (decl.initializer && isGdEvalCall(decl.initializer)) {
            const evalLines = processGdEval(t, decl.initializer);
            if (evalLines && evalLines.length > 0) {
                // First line becomes the RHS of the var declaration
                const firstLine = evalLines[0];
                t.emitter.writeLine(`var ${name}${typeAnnotation} = ${firstLine}`, pos.line, pos.col);
        ...
        const init = decl.initializer
            ...
            : '';
        t.emitter.writeLine(`var ${name}${typeAnnotation}${init}`, pos.line, pos.col);
    }
}
// ---- If Statement ----
export function visitIfStatement(t, node) {
    const pos = t.getLineAndCol(node);
    t.emitter.writeLine(`if ${t.emitExpression(node.expression)}:`, pos.line, pos.col);
    t.emitter.indent();
    ...
    if (node.elseStatement) {
        if (ts.isIfStatement(node.elseStatement)) {
            ...
            t.emitter.writeLine(`elif ${t.emitExpression(node.elseStatement.expression)}:`, elsePos.line, elsePos.col);
            t.emitter.indent();
...
function visitElseChain(t, node) {
    if (ts.isIfStatement(node)) {
        const pos = t.getLineAndCol(node);
        t.emitter.writeLine(`elif ${t.emitExpression(node.expression)}:`, pos.line, pos.col);
        t.emitter.indent();
        visitStatementBody(t, node.thenStatement);
        t.emitter.dedent();
        if (node.elseStatement) {
...
// ---- For Statements ----
export function visitForOfStatement(t, node) {
    const pos = t.getLineAndCol(node);
...
export function visitForStatement(t, node) {
    const pos = t.getLineAndCol(node);
    if (node.initializer) {
        if (ts.isVariableDeclarationList(node.initializer)) {
            visitVariableStatement(t, ts.factory.createVariableStatement(undefined, node.initializer));
        }
        else {
            t.emitter.writeLine(t.emitExpression(node.initializer), pos.line, pos.col);
        }
    }
...
// ---- While Statement ----
export function visitWhileStatement(t, node) {
    const pos = t.getLineAndCol(node);
    t.emitter.writeLine(`while ${t.emitExpression(node.expression)}:`, pos.line, pos.col);
    t.emitter.indent();
    visitStatementBody(t, node.statement);
    t.emitter.dedent();
}
// ---- Statement Body Helper ----
export function visitStatementBody(t, node) {
    const pos = t.getLineAndCol(node);
    if (ts.isBlock(node)) {
