...
import { emitMatchPatternExpr } from "./gd-match.js";
import { emitStatements } from "./statement-body.js";
/**
 ...
 */
export const SWITCH_BREAK_ERROR = '`break` has no GDScript equivalent inside a `match` branch. ' +
 ...
 */
function emitMatchBranch(t, clauses, trailing) {
...
export function visitSwitchStatement(t, node) {
    ...
    const isFallback = (run) => run.some(ts.isDefaultClause);
    const ordered = [
