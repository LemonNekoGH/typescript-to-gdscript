 ...
 */
export function gdFilenameToAnonymousClassName(filePath) {
 ...
 */
export function isAnonymousClassName(name) {
 ...
 */
export function escapeUnderscoreClassName(gdClassName) {
 ...
 */
export function isReportableErrorSeverity(sev) {
 ...
 */
export function isConversionErrorSeverity(sev) {
    return sev === 'error';
}
/**
 * Checks if a TypeScript type is the `int` alias.
 */
export function isIntType(type, checker) {
 ...
 */
export function isFloatType(type, checker) {
    const symbol = type.getSymbol() ?? type.aliasSymbol;
...
export function tsTypeToGdType(type, checker) {
    ...
    if (isIntType(type, checker))
        return 'int';
    if (isFloatType(type, checker))
        return 'float';
    // number -> float
 ...
 * them.
 */
export function isReferenceType(gdType, registry) {
    ...
    if (cleaned === 'any' || cleaned === 'unknown')
        return false;
    if (cleaned.endsWith('[]') ||
    ...
    if (cleaned.includes('.'))
        return false;
    if (registry.isConstructor(cleaned))
        return false;
    return true;
}
