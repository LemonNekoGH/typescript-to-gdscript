 ...
 */
export function isEmptyManifest(data) {
...
export class ProjectCache {
    ...
    startWatching(interval) {
        ...
        const handler = (_path, stats) => {
            ...
            if (this.lastSelfWriteMtime !== null &&
        ...
        this.fileWatcher.on('ready', () => {
            try {
                ...
                if (curr === startupMtime)
                    return;
                if (curr === this.lastSelfWriteMtime)
                    return;
                reload(curr);
            }
            catch {
     ...
     */
    updateTsToGd(tsPath, gdPath, sourceMap, diagnostics, options) {
        const key = this.normKey(tsPath);
     ...
     */
    hasFreshCachedGd(tsPath) {
        const entry = this.data.tsToGd[this.normKey(tsPath)];
        if (!entry?.cachedGdRel)
            return false;
        const abs = join(this.cacheDir, entry.cachedGdRel);
        if (!existsSync(abs))
            return false;
        try {
            return (hashFile(tsPath) === entry.tsHash && hashFile(abs) === entry.gdHash);
        }
        catch {
            return false;
        }
     ...
     */
    promoteCachedGd(tsPath, targetGdPath) {
        if (!this.hasFreshCachedGd(tsPath))
            return false;
        const key = this.normKey(tsPath);
        const entry = this.data.tsToGd[key];
    ...
    isAddonFresh(gdPath, tsPath, dtsPath) {
        const key = this.normKey(gdPath);
        const entry = this.data.addons[key];
        if (!entry)
            return false;
        try {
            return (hashFile(gdPath) === entry.gdHash &&
                existsSync(tsPath) &&
    ...
    updateAddon(gdPath, tsPath, dtsPath) {
        const key = this.normKey(gdPath);
    ...
    isTypingsFresh(sourcePath, dtsPath) {
        const key = this.normKey(sourcePath);
        const entry = this.data.typings[key];
        if (!entry)
            return false;
        try {
            return (hashFile(sourcePath) === entry.sourceHash &&
                existsSync(dtsPath) &&
                hashFile(dtsPath) === entry.dtsHash);
        }
        catch {
    ...
    updateTypings(sourcePath, dtsPath) {
        const key = this.normKey(sourcePath);
     ...
     */
    cleanStale(currentTsFiles, currentAddonFiles, currentTypingSources) {
        if (currentTsFiles) {
    ...
    async doSaveAsync(json, tmpFile) {
    ...
    clearFiles(force = false) {
        ...
        try {
            for (const name of readdirSync(this.cacheDir)) {
                if (keep.has(name))
                    continue;
                if (!force && name.startsWith(inFlightPrefix))
                    continue;
                try {
...
function stripSourceMapContent(sourceMapJson) {
    try {
