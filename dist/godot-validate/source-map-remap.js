 ...
 */
export async function remapError(error, sourceMapJson, tsFilePath) {
    let rawMap = sourceMapJson;
    let mapPath;
    if (!rawMap) {
    ...
    if (rawMap) {
        try {
            ...
            if (!original.source || original.line == null) {
                ...
                const lineMapping = allMappings.find((m) => m.generatedLine === error.line);
                if (lineMapping &&
...
export const VLQ_CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
export function decodeVlq(encoded) {
    const result = [];
    let shift = 0;
    let value = 0;
 ...
 */
export function parseSourceMapMappings(rawMap) {
 ...
 */
export function remapErrorSync(error, sourceMapJson, tsFilePath) {
    if (sourceMapJson) {
        try {
            ...
            const exactMapping = mappings.find((m) => m.generatedLine === error.line && m.generatedColumn <= error.column);
...
//# sourceMappingURL=source-map-remap.js.map