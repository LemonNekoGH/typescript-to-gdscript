 ...
 */
export async function validateGdFiles(options) {
    ...
    for (const gdFileEntry of options.gdFiles) {
        ...
        const relToProject = relative(options.projectRoot, resolvedFile).replace(/\\/g, '/');
 ...
 */
export async function validateGdProject(options) {
    const { signal } = options;
    if (signal?.aborted)
        return { diagnostics: [], godotAvailable: true };
    ...
    if (!existsSync(resolve(options.projectRoot, 'project.godot'))) {
        return {
            diagnostics: [
                {
                    message: `Godot project root has no project.godot ("${options.projectRoot}"). ` +
    ...
    let output;
    try {
        const result = await execFileAsync(options.godotPath, [
            '--headless',
            '--check-only',
            '--path',
            options.projectRoot,
