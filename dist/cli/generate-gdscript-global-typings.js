...
import { writeTypingsIndexDts } from "./helpers.js";
/**
 ...
 */
function detectGodotVersion(docsDirs) {
    const hits = [];
    ...
    if (distinct.length > 1) {
        console.warn(`[WARN] --docs-dir entries resolve to conflicting Godot versions (${distinct.join(', ')}). Using "${hits[0].short}" from ${hits[0].versionFile}. Make sure all docs dirs come from the same Godot tree.`);
    }
    return hits[0].short;
}
/**
 ...
 */
function getDefaultOverrideDir() {
...
export function registerGenerateGdscriptGlobalTypingsCommand(program) {
    program
        .command('generate-gdscript-global-typings')
        .description('Generate TypeScript typings and class registry from Godot docs')
        .option('--docs-dir <dirs...>', 'Godot XML class documentation directories. Accepts one or more paths — Godot ships docs across `doc/classes/` and `modules/<module>/doc_classes/` (notably `modules/gdscript/doc_classes/` for `@GDScript.xml`). Pass every dir whose XMLs you want included; later dirs override earlier ones for same-named classes.')
        .option('--output-dir <dir>', 'Root typings output directory', 'typings')
        .option('--override-dir <dir>', 'User override directory for .d.ts files and non-nullable.json (combined with bundled defaults unless --no-default-overrides is set)')
        ...
        const rawDocsDirs = opts.docsDir;
        if (!Array.isArray(rawDocsDirs) ||
        ...
        for (const d of docsDirs) {
            if (!existsSync(d)) {
                fail(`--docs-dir path does not exist: ${d}\n` +
        ...
        if (!version) {
            fail('Could not detect Godot version from vendor/godot/version.py near any of the given --docs-dir paths');
        }
        ...
        if (opts.defaultOverrides !== false) {
            ...
            if (existsSync(defaultDir))
                overrideDirs.push(defaultDir);
        }
        if (opts.overrideDir) {
            const userDir = resolve(opts.overrideDir);
            if (existsSync(userDir))
                overrideDirs.push(userDir);
        }
...
//# sourceMappingURL=generate-gdscript-global-typings.js.map