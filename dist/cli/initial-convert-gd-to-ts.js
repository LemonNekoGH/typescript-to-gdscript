...
import { injectMissingImportsForProject } from "../converter/gd-to-ts/inject-imports.js";
import { runTsHelpers } from "../converter/gd-to-ts/ts-helpers.js";
import { collectAllSignalHandlers, findSceneFiles } from "../typings/scenes.js";
import { resolveConfig, resolveRegistry } from "../config/index.js";
import { debugLog, resolveFiles, generateAllTypings } from "./helpers.js";
export function registerInitialConvertGdToTsCommand(program) {
    program
        .command('initial-convert-gd-to-ts')
        .description('One-shot bulk GD→TS conversion for the initial migration. Refuses to overwrite existing .ts files unless --force is passed.')
        ...
        .option('--unsafe-use-any', 'Use `any` instead of `unknown` as the fallback for unresolvable types (e.g. gd.getset without a GDScript type or typeof-able value). Less strict but more error-prone.', false)
        .option('--emit-on-error', 'Emit output files even when conversion errors occur', false)
        .option('-f, --force', 'Overwrite existing TypeScript output files. Without this flag the command skips any `.gd` whose mirrored `.ts` already exists and exits non-zero so the migration cannot accidentally clobber hand-edited TS sources.', false)
        .action((files, opts) => {
        ...
        const resolvedFiles = resolveFiles(files.length > 0 ? files : undefined, '.gd', cfg.gdDir, cfg.rootDir, cfg.ignore);
            ...
            for (const diag of result.diagnostics) {
                const prefix = diag.severity === 'error'
                    ...
                    : diag.severity === 'warning'
                        ? 'WARN'
                        : 'INFO';
                collectedErrors.push(`[${prefix}] ${diag.file}:${diag.line}:${diag.column} - ${diag.message}`);
            }
        ...
        if (skippedExistingFiles.length > 0) {
            console.error('');
            console.error(`[ERROR] Refusing to overwrite ${skippedExistingFiles.length} existing TypeScript file(s):`);
            for (const { tsPath, gdPath } of skippedExistingFiles) {
                console.error(`  ${relative(process.cwd(), tsPath)}  (from ${relative(process.cwd(), gdPath)})`);
            }
            console.error('Pass --force to overwrite existing files (this will clobber any hand-edited TypeScript).');
        }
        if (hasErrors || skippedExistingFiles.length > 0)
            process.exit(1);
    });
}
//# sourceMappingURL=initial-convert-gd-to-ts.js.map