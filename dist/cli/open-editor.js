...
import { remapErrorSync } from "../godot-validate/source-map-remap.js";
export function registerOpenEditorCommand(program) {
    program
        .command('open-editor')
        .description('Open a GDScript file in an external editor as the corresponding TypeScript file. ' +
        "Use as Godot's external text editor command.")
        .requiredOption('-f, --file <path>', 'GDScript file path (absolute or res://)')
        .requiredOption('-e, --editor-cmd <cmd>', 'Editor command template. Placeholders: {tsFile}, {tsLine}, {tsCol}')
        .option('-l, --line <n>', 'GDScript line number (from Godot)', '1')
        .option('-c, --col <n>', 'GDScript column number (from Godot)', '1')
        .option('-p, --project <dir>', 'Godot project directory (where tstogd.json is)')
        .action((opts) => {
        ...
        try {
            ...
            console.error(`Could not load tstogd.json from ${projectDir}. ` +
                'Use --project / -p to specify the Godot project directory.');
            process.exit(1);
        }
        // 2. Map .gd → .ts
        let gdPath = opts.file;
        ...
        const gdLine = parseInt(opts.line, 10) || 1;
        const gdCol = parseInt(opts.col, 10) || 1;
        let tsLine = gdLine;
        let tsCol = gdCol;
        ...
        const editorCmd = opts.editorCmd
