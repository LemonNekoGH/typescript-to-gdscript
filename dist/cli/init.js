...
async function stepTstogdJson(rl, cwd) {
...
// ─── Step: tsconfig.json ─────────────────────────────────────
async function stepTsconfig(rl, cwd, config, packagesToInstall) {
    ...
    if (!create)
        return;
    const content = TSCONFIG_TEMPLATE.replace('{{tsDir}}', config.tsDir).replace('{{typingsDir}}', config.typingsDir);
...
async function stepInstallPackages(rl, cwd, packagesToInstall) {
...
// ─── Step: .gdignore ─────────────────────────────────────────
async function stepGdignore(rl, cwd) {
