...
import { parseScene, resourceParser, getSectionAttr } from "./scene-parser.js";
import { SyntaxType } from "../parser/godot-resource/types.js";
import { ANONYMOUS_ADDON_CLASS_NAME, isAnonymousClassName, } from "../converter/common/index.js";
// ─── Naming Helpers ─────────────────────────────────────────
/** Derive a synthetic alias from a scene res:// path, e.g. "res://Level2.tscn" -> "_Level2Tscn" */
export function sceneResPathToAlias(resPath) {
...
export function sceneResPathToOutputFile(resPath) {
...
export function scriptResPathToTreesInterfaceName(resPath) {
    const base = resPath
        .replace(/^res:\/\//, '')
        .replace(/\.gd$/, '');
...
export function sceneResPathToParentsInterfaceName(resPath) {
...
export function nodePathToTypeName(sceneAlias, nodePath) {
...
export function resourceResPathToOutputFile(resPath) {
    return resPath.replace(/^res:\/\//, '') + '.d.ts';
}
/**
 ...
 */
export function computeTsImport(outputDir, fromOutputFile, tsAbsPath) {
...
export function absPathToResPath(absPath, rootDir) {
 ...
 */
export function resolveSignalHandlers(scriptResPath, sceneFiles, registry) {
    ...
    for (const scenePath of sceneFiles) {
        ...
        const scriptNodes = scene.scripts.filter((s) => s.scriptResPath === scriptResPath);
 ...
 */
export function collectAllSignalHandlers(sceneFiles, registry) {
 ...
 */
function resolveUidToResPath(uid, rootDir, autoloadName) {
...
function findFileByName(fileName, dir) {
    let entries;
    try {
...
function findUidFile(targetUid, dir, rootDir) {
    let entries;
    try {
        entries = readdirSync(dir);
    }
 ...
 */
export function parseAutoloads(projectFilePath) {
    ...
    for (const section of root.namedChildren) {
        if (section.type !== SyntaxType.Section)
            continue;
        const sectionIdent = section.namedChildren.find(c => c.type === SyntaxType.Identifier);
        if (!sectionIdent || sectionIdent.text !== 'autoload')
            continue;
        ...
        for (const child of section.namedChildren) {
            ...
            if (pathNode.type !== SyntaxType.Path)
                continue;
            if (valueNode.type !== SyntaxType.String)
                continue;
            ...
            if (resPath.startsWith('uid://')) {
                ...
                if (!resolved)
                    continue; // skip unresolvable UIDs
                resPath = resolved;
            }
...
function findClassInProgram(program, name) {
...
function classExtendsNode(classDecl, checker, program, registry) {
    const visited = new Set();
    // Strategy 1: Walk the TS type checker's base type chain
    let current = checker.getTypeAtLocation(classDecl);
    while (current) {
    ...
    // walk heritage clauses syntactically and search all source files for the base class.
    {
        ...
        const syntacticVisited = new Set();
        while (baseName) {
            if (syntacticVisited.has(baseName))
                break;
            syntacticVisited.add(baseName);
            if (baseName === 'Node')
                return true;
            if (registry?.hasClass(baseName))
                return registryExtendsNode(baseName, registry);
...
function registryExtendsNode(name, registry) {
    let gdClass = name;
    while (gdClass) {
        if (gdClass === 'Node')
            return true;
...
export function scanTsFilesForClasses(program, files, baseDir, scriptClassMap, registry, 
/**
...
isAddon = false) {
    ...
    for (const filePath of files) {
        ...
        // detected here.
        const fileScopeEnums = [];
        const fileScopeInnerClasses = [];
        for (const stmt of sourceFile.statements) {
            ...
            if (ts.isClassDeclaration(stmt) && stmt.name) {
                ...
                if (isExported)
                    continue; // script class \u2014 collected separately
                let extendsName;
 ...
 */
export function parseGdResourceType(filePath) {
    try {
        ...
        for (const section of root.namedChildren) {
            if (section.type !== SyntaxType.Section)
                continue;
            const sectionIdent = section.namedChildren.find(c => c.type === SyntaxType.Identifier);
            if (!sectionIdent || sectionIdent.text !== 'gd_resource')
                continue;
 ...
 */
function parseGdResourceUidHeader(filePath) {
    const content = readFileSync(filePath, 'utf-8');
    const root = resourceParser.parse(content);
    for (const section of root.namedChildren) {
        if (section.type !== SyntaxType.Section)
        ...
        if (ident?.text !== 'gd_scene' && ident?.text !== 'gd_resource')
            continue;
 ...
 */
export function resolveResourceUid(absPath) {
    try {
...
export const ASSET_EXTENSION_MAP = {
    // Images / Textures
...
export function findProjectFiles(dir, rootDir, ignore, extensions) {
    const results = [];
    try {
...
export function findSceneFiles(dir, rootDir, ignore) {
    return findProjectFiles(dir, rootDir, ignore, new Set(['.tscn']));
}
export function findAssetFiles(dir, rootDir, ignore) {
