import { gdDocToPlain } from "./godot-registry.js";
import { godotTypeToTs, isNullableGodotType, widenParamType, sanitizeClassName, } from "./type-mapping.js";
...
export { sanitizeClassName } from "./type-mapping.js";
// ─── JSDoc generation ────────────────────────────────────────────
/**
 ...
 */
export function emitJsDoc(description, indent = '  ') {
    if (!description)
        return [];
...
export function emitMethodSignature(method, ctx, nonNullMembers) {
    const lines = [];
    // JSDoc
    ...
    const methodName = needsQuoting(method.name)
        ? `'${method.name}'`
        : method.name;
    lines.push(`  ${staticPrefix}${methodName}(${params.join(', ')}): ${returnType}${nullable};`);
    return lines;
}
// ─── Operator Overloads ──────────────────────────────────────────
 ...
 */
export function emitOperatorOverloads(operators, ctx) {
    const lines = [];
 ...
 */
export function generateClassDeclaration(cls, ctx, dictOnlyOverrides, 
...
inheritedMethodNames) {
    ...
    const nonNullMembers = ctx.nonNullableMembers.get(cls.name) ?? new Set();
    // Properties
 ...
 */
export function generateValueTypeDeclaration(cls, dictMembers, ctx) {
    const lines = [];
    const className = sanitizeClassName(cls.name);
    // Collect own member names to avoid overriding with never
    const ownMembers = new Set();
    for (const prop of cls.properties)
        ownMembers.add(prop.name);
    for (const method of cls.methods)
        ownMembers.add(method.name);
 ...
 */
export function generateConstructorInterface(cls, className, returnType, ctx) {
 ...
 */
export function generateInterfaceDeclaration(cls, tsName, ctx) {
    const lines = [];
    // Collect variant convert types from single-parameter "from" constructors
    const variantConvertTypes = [];
    for (const ctor of cls.constructors) {
