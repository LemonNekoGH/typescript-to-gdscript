import { nodePathToTypeName, sceneResPathToTreeName } from "./scene-utils.js";
/**
 * Build a tree structure from the flat node list returned by parseScene().
 */
export function buildNodeTree(sceneData) {
    ...
    for (const script of sceneData.scripts) {
        ...
        if (node)
            node.scriptResPath = script.scriptResPath;
    }
 ...
 */
export function collectDescendantPaths(node, sceneAlias, instancedSceneTreeNames, extendedInstancedNodes) {
    const paths = [];
