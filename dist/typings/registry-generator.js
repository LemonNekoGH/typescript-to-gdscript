 ...
 */
export function deriveConstructorTypes(classes) {
    const result = [];
 ...
 */
export function deriveOperatorTypes(classes) {
    const result = [];
    for (const [name, cls] of classes) {
        if (name.startsWith('@'))
            continue;
        if (NATIVE_OPERATOR_TYPES.has(name))
            continue;
 ...
 */
export function generateRegistryData(classes, gdscriptCls) {
    const registry = {
        version: '',
        classes: {},
        globalFunctions: [],
 ...
 */
export function parseGodotVersion(versionPyPath) {
