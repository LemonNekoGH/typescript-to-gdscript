...
import { GodotResourceParser } from "../parser/godot-resource/index.js";
 ...
 */
export function getSectionAttr(section, name) {
 ...
 */
export function getSectionProp(section, name) {
    for (const child of section.namedChildren) {
 ...
 */
export function getSectionStringArray(section, name) {
    for (const child of section.namedChildren) {
 ...
 */
export function getConstructorFirstArg(node) {
    if (node.type !== SyntaxType.Constructor)
        return undefined;
 ...
 */
export function getSectionExtResource(section, propName) {
    for (const child of section.namedChildren) {
        if (child.type !== SyntaxType.Property)
            continue;
        const parts = child.namedChildren;
 ...
 */
function getSectionSubResource(section, propName) {
    for (const child of section.namedChildren) {
        if (child.type !== SyntaxType.Property)
            continue;
        const parts = child.namedChildren;
 ...
 */
function getSectionPropsWithPrefix(section, prefix) {
    const results = [];
    for (const child of section.namedChildren) {
        if (child.type !== SyntaxType.Property)
            continue;
 ...
 */
function getSubResourceId(valueNode) {
    if (valueNode.type !== SyntaxType.Constructor)
        return undefined;
 ...
 */
function getExtResourceId(valueNode) {
    if (valueNode.type !== SyntaxType.Constructor)
        return undefined;
    const ctorIdent = valueNode.namedChildren.find(c => c.type === SyntaxType.Identifier);
    if (ctorIdent?.text !== 'ExtResource')
 ...
 */
export function collectTileMapScenes(nodeSection, subResources) {
    const extIds = [];
    ...
    if (!tileSet || tileSet.type !== 'TileSet')
        return extIds;
    ...
    for (const { valueNode } of sourceProps) {
        ...
        if (!source || source.type !== 'TileSetScenesCollectionSource')
            continue;
 ...
 */
export function parseScene(filePath) {
    ...
    function collectChildren(scriptNode, nodePath) {
        const children = [];
        for (const other of nodes) {
            ...
            if (!nodeType && !instanceSceneResPath)
                continue;
            const otherPath = other.parent === '.'
                ? other.name
    ...
    for (const node of nodes) {
        ...
        if (!extRes || extRes.type !== 'Script' || !extRes.path.endsWith('.gd'))
            continue;
        const nodePath = node.parent === '' ? '.' : node.parent === '.' ? node.name : `${node.parent}/${node.name}`;
        scripts.push({
    ...
    for (const node of nodes) {
        if (!node.type)
            continue;
        const fullPath = node.parent === '' ? '.' : node.parent === '.' ? node.name : `${node.parent}/${node.name}`;
        nodeTypes.set(fullPath, node.type);
    }
    // Build instanced nodes map (full path -> instanced scene res:// path)
    const instancedNodes = new Map();
    for (const node of nodes) {
