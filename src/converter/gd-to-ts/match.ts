 ...
 */
function hasTsStatement(emitted: string): boolean {
...
function emitSimpleMatchAsSwitch(
  ...
  if (bodyNode) {
    ...
    const sections = bodyNode.namedChildren.filter(
      (s) => s.type === SyntaxType.PatternSection,
    );
...
export function emitMatchStatement(
  node: SyntaxNode,
  ctx: GdToTsContext,
  depth: number,
): string {
