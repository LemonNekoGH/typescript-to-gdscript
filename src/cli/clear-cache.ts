...
import { ProjectCache, isEmptyManifest } from '../cache/index.ts';
 ...
 */
function whyNotCleared(cacheFile: string): string | null {
  ...
  if (!isEmptyManifest(data)) return 'the manifest still holds cache entries';
  return null;
}

function cacheDirEntries(cacheDir: string): string[] {
  try {
 ...
 * separately instead.
 */
function leftovers(cacheDir: string): string[] {
  return cacheDirEntries(cacheDir).filter(
    (name) =>
      name !== '.gdignore' &&
      name !== 'cache.json' &&
...
function sparedTmpFiles(cacheDir: string): string[] {
  return cacheDirEntries(cacheDir).filter((name) =>
    name.startsWith('cache.json.tmp-'),
  );
}
...
export function registerClearCacheCommand(program: Command): void {
  program
    ...
    .option(
      '--force',
      'also remove .gdignore and any cache.json.tmp-* files. A tmp file is ' +
    ...
    .action((opts: { force?: boolean }) => {
      ...
      try {
        ...
        console.error(
          `Failed to clear cache: ${cacheDir}\n` +
      ...
      if (problem || remaining.length > 0) {
        console.error(
          `Cache was not fully cleared: ${cacheDir}` +
            (problem ? `\n${cacheFile}: ${problem}.` : '') +
            (remaining.length > 0
