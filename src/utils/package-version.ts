...
function readVersion(): string {
  ...
  try {
    ...
    throw new Error(
      `Cannot read the package version: failed to read ${pkgPath} (${(err as Error).message})`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Cannot read the package version: ${pkgPath} is not valid JSON (${(err as Error).message})`,
    );
  }

  if (typeof version !== 'string' || version === '') {
    throw new Error(
      `Cannot read the package version: ${pkgPath} has no "version" field`,
    );
  }
  return version;
}
